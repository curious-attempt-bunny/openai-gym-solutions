...
def state_key(state):
    return "{},{}".format( \
        normalize(env.observation_space.high[0], env.observation_space.low[0], state[0], 45), \
        normalize(env.observation_space.high[1], env.observation_space.low[1], state[1], 45) \
    )
...
for i_episode in range(15000):
    ...
    for t in range(200):
        ...
        q_max_next = 0.0
        if not done:
            q_max_next = np.max(q[state_key(state_next)])
